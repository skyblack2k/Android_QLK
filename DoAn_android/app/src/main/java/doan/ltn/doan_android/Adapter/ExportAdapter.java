...
public class ExportAdapter extends RecyclerView.Adapter<ExportAdapter.ViewHolder> {
    ...
    private ArrayList<Export> list;
    private ItemClickListener itemClickListener;

    public ExportAdapter(ArrayList<Export> list, ItemClickListener itemClickListener) {
    this.list=list;
    ...
    @Override
    public void onBindViewHolder(@NonNull ExportAdapter.ViewHolder viewHolder, int i) {
        Export item=list.get(i);
        viewHolder.ma.setText(String.valueOf(item.getID()));
        viewHolder.name.setText(String.valueOf(item.getCuaHangID()));
        viewHolder.date.setText(String.valueOf(item.getNgayLap()));
        viewHolder.icon.setBackgroundResource(R.drawable.deliverytruck_13);
